...
namespace Lut
{
    ...
    CubeLut::LutState CubeLut::LoadCubeFile(std::ifstream &infile)
    {
        ...
        for (int i = 0; i < 255; i++)
        {
            const char inc = static_cast<char>(infile.get());
        ...
        if (status == LutState::OK && (DomainMin.B >= DomainMax.B || DomainMin.G >= DomainMax.G || DomainMin.R >= DomainMax.R))
        ...
        std::visit([&]<typename T0>(T0 &tb)
	            ...
	            if constexpr (std::is_same_v<T, Table1D>)
	            {
                    for (size_t i = 0; i < tb.Length() && status == LutState::OK; i++)
                    {
    ...
    CubeLut::LutState CubeLut::SaveCubeFile(std::ofstream &outfile)
    {
        ...
        std::visit([&]<typename T0>(const T0 &tb)
            ...
            if constexpr (std::is_same_v<T, Table1D>)
            {
                ...
                for (size_t i = 0; i < n && outfile.good(); i++)
                {
            ...
            else if constexpr (std::is_same_v<T, Table3D>)
            {
                ...
                for (size_t b = 0; b < n && outfile.good(); b++)
                {
                    for (size_t g = 0; g < n && outfile.good(); g++)
                    {
                        for (size_t r = 0; r < n && outfile.good(); r++)
                        {
